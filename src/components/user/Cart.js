...
const Cart = ({ cartItems, cartItemCount, removeFromCart, fetchCartItems }) => {
            ...
            <Card sx={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: '#f9f9f9' }}> 
              ...
              <CardContent sx={{ flexGrow: 1 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', mt: 2 }}>
                  <Button
                    size="small"
                    onClick={() => handleQuantityChange(item.productId._id, Math.max(1, (quantities[item.productId._id] !== undefined ? quantities[item.productId._id] : item.quantity) - 1))}
                  >
                    -
                  </Button>
                  <TextField
                  ...
                  <Button
                    size="small"
                    onClick={() => handleQuantityChange(item.productId._id, (quantities[item.productId._id] !== undefined ? quantities[item.productId._id] : item.quantity) + 1)}
                  >
                    +
                  </Button>
                  <Button
                    size="small"
                    onClick={() => handleUpdateQuantity(item.productId._id, quantities[item.productId._id] !== undefined ? quantities[item.productId._id] : item.quantity)}
                    sx={{ ml: 1 }}
                  >
                    Update
                  </Button>
